					...
					<div class="col-15 col-xl-12">
							<div class="card">
								<div class="card-header">
									<a href="tambah_kategori.php"><button type="button" class="btn btn-primary btn-lg">Tambah kategori</button></a>
								</div>
								<table class="table table-bordered">
									<thead>
										<tr>
											<th style="width:8%;">No</th>
											<th style="width:18%;">Nama Kategori</th>
									...
									<tbody>
										<?php
                      include "koneksi.php";
                      $qry_buku=mysqli_query($conn,"select * from kategoribuku");
                      $no=0;
                      while($data_buku=mysqli_fetch_array($qry_buku)){
                      $no++;?>
                      <tr class="text-xs font-weight-bold">
                          <td class="align-middle text-left"><?=$no?></td>
                          <td class="align-middle text-left"><?=$data_buku['NamaKategori']?></td>
                          <td class="text-xs font-weight-bold">
                          <a class="btn btn-success" href="ubah_kategori.php?id_kategori=<?=$data_buku['id_kategori']?>" >Ubah</a>
                          <a href="hapus_kategori.php?id_kategori=<?=$data_buku['id_kategori']?>" onclick="return confirm('Apakah anda yakin menghapus data ini?')" class="btn btn-danger">Hapus</a></td>
                      </tr>
                      <?php
                      }
                      ?>
